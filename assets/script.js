...
theme.adjustMenu = function () {
  $(function() {
    var sticknavigation = $(".sticky-navigation");
    if(sticknavigation.length > 0){
  ...
  $(window).on('scroll', function() {
    ...
    $('.animate-section-div').each(function() {
      $(this).find(ani_sect).removeClass('load loaded');
      if ($(this).isInViewport()) {
        $(this).find(ani_sect).addClass('go').removeClass('stop');
      } else {
        $(this).find(ani_sect).addClass('stop').removeClass('go');
      }
    });
  });
  var scrollPos = 0;
  window.addEventListener('scroll', function(){
    if ((document.body.getBoundingClientRect()).top > scrollPos) {
      ani_sect.addClass('up').removeClass('down');
    } else {
      ani_sect.addClass('down').removeClass('up');
    }
...
theme.product_options = function ($container) {
  ...
  selectCallback = function(variant) {
    ...
    var productForm = $('form.product_form-' + sectionId ),
        ...
        productInventoryIncoming = $('#product-select-' + sectionId + ' option:selected').data('inc'),
        productInventoryIncomingDate = $('#product-select-' + sectionId + ' option:selected').data('inc_date'),
        ...
        if (variant && variant.available == true) {
          ...
          $(productAdd).val(theme.language.products_product_sold_out).addClass('disabled outline').attr('disabled', true);
          $(productStock).hide();
    ...
    if (variant) {
      ...
      if (variant.inventory_management) {
        if (productInventoryAmt > 0 && productInventoryAmt < productInventoryCutoff) {
          if(productInventoryIncoming == true) {
            $(productInventory).html('<h4>' + theme.language.products_general_inv_msg_1 + '&nbsp;' + productInventoryAmt + '&nbsp;' + theme.language.products_general_inv_msg_2 + '<br/><span class="error-text"><small><span class="icon icon-bell"></span> ' + productInventoryIncomingDate + '</small</span></h4>');
          } else {
          ...
          if (variant.available) {
            ...
            if(productInventoryIncoming == true) {
              $(productInventory).html('<h4>' + theme.language.products_product_sold_out + '<br/><span class="error-text"><small><span class="icon icon-bell"></span> ' + productInventoryIncomingDate + '</small></span></h4>');
            } else {
      ...
      if (variant.unit_price_measurement) {
        if (variant.unit_price_measurement.reference_value != 1) {
          $(productOffer).html('<h4>' + Shopify.formatMoney(variant.unit_price,theme.moneyFormat) + theme.language.products_general_per + variant.unit_price_measurement.reference_value + variant.unit_price_measurement.reference_unit + '</h4>').removeClass('hidden');
        } else {
          $(productOffer).html('<h4>' + Shopify.formatMoney(variant.unit_price,theme.moneyFormat) + theme.language.products_general_per + variant.unit_price_measurement.reference_unit + '</h4>').removeClass('hidden');
        }
      } else {
        $(productOffer).empty().addClass('hidden');
      }
      if (variant && variant.featured_media) {
        ...
        var mySwiperIndex = $newImage.attr('data-media-index');

        if(galleryTopSwiper) {
        galleryTopSwiper.slideTo(mySwiperIndex, 300);
        }

        $newImage.removeClass('hidden').trigger('mediaVisible');
        $otherImages.addClass('hidden').trigger('mediaHidden');
      }
    } else {
      $(productSKU).empty();
      $(productInventory).empty();
  ...
  var single_option_selectors = $('.single-option-selector');

  if(single_option_selectors.length) {
    single_option_selectors.each(function() {
      if($(this).val() == 'Default Title') {
      	$(this).addClass('nowSelect2').closest('.selector-wrapper').hide();
      }
    });
    single_option_selectors.not('.nowSelect2').select2();
  }
  ...
  var Thb = function (){
    ...
    $otherThumbs.addClass('hidden').trigger('mediaHidden').blur();
    $newThumb.removeClass('hidden').trigger('mediaVisible').focus();
    $('#product-id-' + sectionId + ' .selector-wrapper').addClass('active');
  }
  //   $('.thumb-image-' + sectionId).keypress(Thb).click(Thb);
  if($('#thumbnails-'+ sectionId).length ) {
   var thumbSwiper = new Swiper ('#thumbnails-'+ sectionId, {
      ...
      spaceBetween: 6,
      navigation: {
        nextEl: '#swiper-button-next-thumbs-' + sectionId,
        prevEl: '#swiper-button-prev-thumbs-' + sectionId,
      },
      on: {
        init: function() {
  ...
  if($('.gallery-top-'+ sectionId).length ) {
    galleryTopSwiper = new Swiper('.gallery-top-'+ sectionId, {
      spaceBetween: 0,
      slidesPerView: 1,
      navigation: {
        nextEl: '#swiper-button-next-gallery-'+ sectionId,
        prevEl: '#swiper-button-prev-gallery-'+ sectionId
      },
      thumbs: {
        swiper: thumbSwiper
      }
...
theme.product_media = function ($container) {
  ...
  $(productMediaTypeVideo, this.$container).each(function() {
    var $el = $(this);
    theme.product_video.init($el, sectionId);
  });
  ...
  theme.product_model.init($modelViewerElements, sectionId);
  var self = this;
  $(document).on('shopify_xr_launch', function() {
    var $currentMedia = $(
      self.productMediaWrapper +
      ':not(.' +
      self.classes.hidden +
      ')',
      self.$container
...
theme.qtyinput_quantity = function ($container) {
  ...
  $('.qtyplus_' + sectionId + ',.qtyplus_multi').click(function(e) {
    ...
    var fieldName = $(this).attr('field'),
        ...
        priceid = parseInt($('input[id='+pricefieldName+']').attr('price')),
        key = document.getElementById(fieldName),
    ...
    $('input[id='+pricefieldName+']').val(newVal * priceid );
  });
...
//       $addToCartForm.find(_config.addToCartBtnSelector).after(feedback);
        $addToCartForm.find('.detail-price').after(feedback);
        ...
        $(_config.addToCartFormSelector).submit(function(e) {
          ...
          $.ajax({
            ...
            success: function(itemData) {
              ...
              var $shop_currency = '{{ cart.currency.iso_code }}';
              ajaxLoadPage();
              ...
              $.getJSON(_config.shopifyAjaxCartURL, function(cart) {
                ...
                var cart_notification = $('#cart-notification');
                if(cart_notification.length) {
                  var $product__imageEl = cart_notification.find('.cart-notification-product__image');
                  var $product__nameEl = cart_notification.find('.cart-notification-product__name');
                  var $product__optionWrapperEl = cart_notification.find('.cart-notification-product__options');
                  var $cart_notificationViewAllBtnEl = cart_notification.find('#cart-notification-button');

                  $product__nameEl.html(itemData.title);
                  $product__imageEl.attr('src',itemData.featured_image.url);
                  $product__imageEl.attr('alt',itemData.featured_image.alt);

                  $product__optionWrapperEl.html(' ');
                  if(itemData.options_with_values.length) {
                    for(var i=0;i< itemData.options_with_values.length;i++) {
                      var optionData = itemData.options_with_values[i];
                      var name = optionData.name;
                      var value  = optionData.value ;

                      if(value != 'Default Title') {
                        var optionRow = '<div class="cart-notification-product__option"><dt>'+name+': </dt><dd>'+value+'</dd></div>';
                        $product__optionWrapperEl.append(optionRow);
                      }
                    }
                  }

                  $cart_notificationViewAllBtnEl.html('View my cart ('+cart.item_count +')');
...
theme.smooth_scroll = function () {
  $("a[href^='#']").not('.swiper-slide').not('.nav-link').not('.hidetab').on('click', function(e) {
    if (this.hash !== '') {
      e.preventDefault();
      var stickyHeight = $('.header-toolbar.sticky').outerHeight();
      ...
      var hash = this.hash;
      $('html, body').animate({
        scrollTop: ($(hash).offset().top - stickyHeightTotal)
      }, 900, 'swing');
    }
  });
};
...
theme.live_search = function ($container) {
  ...
  var currentAjaxRequest=null,
      searchForms=$('form.search-form-' + sectionId).css("position","relative").each(function(){
        var t=$(this).find('input[name="q"]'),
            e=t.position();
        $('<ul class="search-results inline text-left unformatted"></ul>').css({position:"absolute",left:"0px"}).appendTo($(this)).hide(),t.attr("autocomplete","off").bind("keyup change",function(){
          var t=$(this).val(),
              e=theme.routes_search_url + '?q=' +t,
              a=$(this).closest('form').find('.search-results');
          t.length>3&&t!=$(this).attr('data-old-term')&&($(this).attr('data-old-term',t),null!=currentAjaxRequest&&currentAjaxRequest.abort(),currentAjaxRequest=$.getJSON(e+'&view=json',function(t){
            a.empty(),0==t.results_count?a.hide():(
              $.each(t.results,function(t,e){
                var s=$('<a></a>').attr('href',e.url).attr('title',e.title),
                    col='six',
                    p='',
                    v='';
                ...
                if (e.price != undefined){
                  p =  '<span class="product-price"><h6>'+Shopify.formatMoney(e.price,theme.moneyFormat)+'</h6></span>'
                }
                if (e.vendor != undefined){
                  v =  '<span class="vendor"><small>'+e.vendor+'</small></span>'
                }
                e.thumbnail.indexOf('no-image')<=0&&s.append('<span class="thumbnail"><img src="'+e.thumbnail+'" /></span>').addClass('with-image'),
                  s.append('<span><span class="h5">'+e.title+'</span>' + p + v + '</span>' ),
                  s.wrap('<li class="' + col + ' relative ' + e.category + '"></li>'),
                  a.append(s.parent())
              }),t.results_count>10&&a.append('<li class="see-all twelve text-center"><a href="'+e+'">'+theme.language.general_search_see_results+' ('+t.results_count+")</a></li>"),a.fadeIn(200))
          }))
        })
      });
...
theme.recently_viewed = function () {
  Shopify.Products=(function(){var a={howManyToShow:5,howManyToStoreInMemory:5,wrapperId:"recently-viewed-products",templateId:"recently-viewed-product-template",onComplete:null};var c=[];var h=null;var n;var d=null;var e=0;var b={configuration:{expires:90,path:"/",domain:window.location.hostname},name:"shopify_recently_viewed",write:function(i){$.cookie(this.name,i.join(" "),this.configuration)},read:function(){var i=[];var j=$.cookie(this.name);if(j!==null){i=j.split(" ")}return i},destroy:function(){$.cookie(this.name,null,this.configuration)},remove:function(k){var j=this.read();var i=$.inArray(k,j);if(i!==-1){j.splice(i,1);this.write(j)}}};var f=function(){h.addClass('active');if(a.onComplete){try{a.onComplete()}catch(i){}}};var g=function(){if(c.length&&e<a.howManyToShow){$.ajax({dataType:"json",url:"/products/"+c[0]+".js",cache:false,success:function(i){d.tmpl(i).appendTo(n);c.shift();e++;g()},error:function(){b.remove(c[0]);c.shift();g()}})}else{f()}};return{resizeImage:function(m,j){if (m!= null){var img = new Image();img.onload = function() {if (this.height > this.width) {$('img[data-img="' + m + '"]').addClass('vertical');}};img.src = m;if(j==null){return m}if(j=="master"){return m.replace(/http(s)?:/,"")}var i=m.match(/\.(jpg|jpeg|gif|png|bmp|bitmap|tiff|tif)(\?v=\d+)?/i);if(i!=null){var k=m.split(i[0]);var l=i[0];return(k[0]+"_"+j+l).replace(/http(s)?:/,"")}else{return null}}else{var m = 'https://cdn.shopify.com/s/images/admin/no-image-2048x2048.gif';return m;}},showRecentlyViewed:function(i){var i=i||{};$.extend(a,i);c=b.read();d=$("#"+a.templateId);h=$("#"+a.wrapperId);n=$("#"+a.wrapperId+' .row');a.howManyToShow=Math.min(c.length,a.howManyToShow);if(a.howManyToShow&&d.length&&h.length){g()}},getConfig:function(){return a},clearList:function(){b.destroy()},recordRecentlyViewed:function(l){var l=l||{};$.extend(a,l);var j=b.read();if(window.location.pathname.indexOf("/products/")!==-1){var k=window.location.pathname.match(/.*\/(.*)$/)[1];var i=$.inArray(k,j);if(i===-1){j.unshift(k);j=j.splice(0,a.howManyToStoreInMemory)}else{j.splice(i,1);j.unshift(k)}b.write(j)}}}})();
};
theme.multi = function () {
  $(function() {
    var a = $('.currencies'),
        ah = $('header .currencies'),
        af = $('footer .currencies'),
        g = a.find('select'),
        e = a.find('a'),
        b = a.find('ul'),
        c = b.find('a');
    c.click(function(i) {
      i.stopPropagation();
      ah.find('a[data-multi="' + $(this).data('multi') + '"]').html('<small>' + $(this).data('header_symbol') + '</small>').removeClass('active');
      af.find('a[data-multi="' + $(this).data('multi') + '"]').html('<small>' + $(this).data('footer_symbol') + '</small>').removeClass('active');
...
theme.HomeSlideshow = (function() {
  function HomeSlideshow(container) {
    ...
    var mySwiper = new Swiper (Carousel, {
      ...
      on: {
        init: function() {
          var newHeight = Carousel.find('.home-slideshow-image .swiper-slide-active img').data('aspectratio');
          Carousel.find('.home-slideshow-image.swiper-wrapper,.home-slideshow-image .swiper-slide-active').css({height:CarouselWidth/newHeight})
        }
      }
    });
    mySwiper.on('slideChangeTransitionStart', function () {
      Carousel.find('.swiper-slide-active .lazyloading').addClass('lazyload');
      var newHeight = Carousel.find('.home-slideshow-image .swiper-slide-active > div').height();
      $('.home-slideshow-image.swiper-wrapper').css({height:newHeight})
    });
    mySwiper.on('resize', function () {
      Carousel.find('.home-slideshow-image .swiper-slide-active').css('height','auto')
    });
    HomeSlideshow.prototype.onBlockSelect = function(evt) {
      ...
      if ($('#slideshow-'+sectionId).hasClass('swiper-enabled')) {
        ...
        var newHeight = Carousel.find('.home-slideshow-image .swiper-slide-active img').data('aspectratio');
        $('#slideshow-carousel-' + sectionId + ' .home-slideshow-image.swiper-wrapper').css({height:CarouselWidth/newHeight})
      }
    };
  }
...
theme.FooterSection = (function() {
  function FooterSection(container) {
    var $container = this.$container = $(container),
        sectionId = this.sectionId = $container.attr('data-section-id');
    $(function() {
      var g = $('footer').find('select'),
          e = $('footer').find('a.styledSelect'),
          b = $('footer').find('ul.options'),
          c = b.find('a');
      e.click(function(i) {
        i.stopPropagation();
        $('a.styledSelect').not(this).next('ul.options').hide().removeClass('active');
...
theme.Collection = (function() {
  function Collection(container) {
    var $container = this.$container = $(container),
        sectionId = this.sectionId = $container.attr('data-section-id'),
        stickyHeight = $('.header-toolbar.sticky').outerHeight();
    if (stickyHeight === undefined) {
      var stickyHeightTotal = 0;
    } else {
      var stickyHeightTotal = stickyHeight;
    ...
    if(window.location.hash) {
      $('html, body').animate({scrollTop: $('#collection').offset().top -stickyHeightTotal}, 0);
    }
    var filter = {
      length:function (e) {
        $('.filter ul').each(function() {
          ...
          if (items > 5) {
            ...
            $(this).append('<li class="open-more-container"><a href="javascript:void(0)" class="open-more" title="' + theme.language.collections_general_open + '" aria-controls="' + theme.language.collections_general_open + '" role="button" aria-label="' + theme.language.collections_general_open + '">' + theme.language.collections_sidebar_more + '</a></li>');
          }
        });
      ...
      cart:function (e) {
        $.getJSON('/cart.js', function(response) {
          ...
          for (i = 0, len = items.length; i < len; i++) {
            ...
            $('#variant_' + thisId).addClass('item-in-cart');
          }
          return;
        });
      }
    ...
    function cart_change(e) {
      ...
      var this_item = $(this).closest('.item'),
          ...
          success = this_item.find('.error'),
          cartCountSelector = $('.cartCountSelector'),
          cartTotalSelector = $('.cartTotalSelector');
      if ($(this_item).hasClass('item-in-cart')) {
      ...
      $.ajax({
        ...
        success: function(cart) {
          ...
          $(success).show().html(theme.language.products_product_ajax_updated);
          if (qty > 0) {
            $(this_item).addClass('item-in-cart');
          } else {
            $(this_item).removeClass('item-in-cart');
          }
          $.getJSON('/cart.js', function(cart) {
            if (cartCountSelector && $(cartCountSelector).length) {
              var value = $(cartCountSelector).html() || '0';
              $(cartCountSelector).html(value.replace(/[0-9]+/,cart.item_count)).removeClass('hidden');
    ...
    $('#collection').on('change, mouseup, keyup', '.item-qty', cart_change).on('change, click', '.qtybtn', cart_change);
    var coll = {
      options:function (e) {
    ...
    var ajaxLoadPage = function (url) {
      $.ajax({
        ...
        complete: function (data) {
          ...
          $('html, body').animate({scrollTop: $('#collection').offset().top -stickyHeightTotal}, 900);
        }
      });
    };
    filter.length();
...
theme.Product = (function() {
  function Product(container) {
    ...
    var loadProductRecommendationsIntoSection = function() {
      ...
      var requestUrl = theme.routes_product_recommendations_url + '?section_id=product-recommendations&limit=' + limit + '&product_id=' + productId;
      ...
      request.onload = function() {
        if (request.status >= 200 && request.status < 300) {
          ...
          if (productRecommendationsSection.children.length < 2) {
  ...
  function Cart(container) {
    ...
    function cart_change(e) {
      ...
      var this_item = $(this).closest('.item'),
          variant_id = this_item.data('product-id'),
          qty =  this_item.find('.item-qty').val(),
          this_item_badge = this_item.find('.badge-total'),
      ...
      $.ajax({
        ...
        success: function(cart) {
          ...
          var value = $(cartCountSelector).html(),
              ajaxLoadPage = function (url) {
                $.ajax({
                  ...
                  complete: function (data) {
                    $('#cart-dropdown-span').html($('#cart-dropdown-span', data.responseText).html());
                    $('.discount-cart').html($('.discount-cart', data.responseText).html());
          ...
          if (qty != '') {
            ...
            if (qty == 0) {
              ...
              if (cart.item_count == 0) {
                ...
                $(cartCountSelector).addClass('hidden');
              }
            } else {
              $.each(cart.items,function(index,row) {
                if ( variant_id == row.key ) {
                  if ((this_item_compare * row.quantity) > row.final_line_price) {
                    ...
                    this_item_badge.html(theme.language.products_product_savings + '<br/>' + Shopify.formatMoney((this_item_compare * row.quantity) - row.final_line_price, theme.moneyFormat).replace(/((\,00)|(\.00))$/g, ''));
                    ...
                    $('.discount-cart').html('...');
                    this_item_badge.html(theme.language.products_product_savings + '<br/>' + Shopify.formatMoney(row.original_line_price - row.final_line_price, theme.moneyFormat).replace(/((\,00)|(\.00))$/g, ''));
                  } else {
          ...
          if (cartTotalSelector && $(cartTotalSelector).length) {
            ...
            $(cartTotalSelector).html(Shopify.formatMoney(cart.total_price, theme.moneyFormat));
          ...
          // Theme Custom - Shipping message.
          var shippingThresholdValue = $('.template-cart .shipping-message .shipping-threshold-value').data('shipping-threshold'),
              cartTotalValue = cart.total_price / 100
              amountUntilThreshold = ((shippingThresholdValue * 100).toFixed(2) - (cartTotalValue * 100).toFixed(2));

          if (cartTotalValue >= shippingThresholdValue) {
            $('.template-cart .under-threshold').addClass('hide-message');
            $('.template-cart .threshold-reached').removeClass('hide-message');
          } else {
            $('.template-cart .threshold-reached').addClass('hide-message');
            $('.template-cart .amount-until-threshold').html(Shopify.formatMoney(amountUntilThreshold, theme.moneyFormat));
            $('.template-cart .under-threshold').removeClass('hide-message');
          }
        }
      });
    }
    $('#cart-table').on('change, mouseup, keyup', '.item-qty', cart_change).on('change, click', '.qtybtn', cart_change);
  }
  return Cart;
})();
theme.Blog = (function() {
...
theme.Contact = (function() {
  function Contact(container) {
    var $container = this.$container = $(container),
        sectionId = this.sectionId = $container.attr('data-section-id'),
        form_send = theme.language.contact_form_send,
        stickyHeight = $('.header-toolbar.sticky').outerHeight();
    if (stickyHeight === undefined) {
      var stickyHeightTotal = 0;
    } else {
    ...
    if(window.location.hash) {
      $('html, body').animate({scrollTop: $('#contact_form').offset().top -stickyHeightTotal}, 0);
    }
...
theme.HomeIngredientsSlideshow = (function() {
  function HomeIngredientsSlideshow(container) {
    var $container = this.$container = $(container),
        sectionId = this.sectionId = $container.attr('data-section-id'),
        Carousel = this.Carousel = $container.find('.ingredients_slider .swiper-container'),
        next = '#swiper-button-next-'+sectionId,
        prev = '#swiper-button-prev-'+sectionId;
    var mySwiper = new Swiper (Carousel, {
      autoHeight: false,
...
//     HomeIngredientsSlideshow.prototype.onBlockSelect = function(evt) {
//       console.log(evt);
//       debugger;
//       var sectionId = this.sectionId,
//           mySwiperIndex = $(evt.target).data('swiper-slide-index');
//       if ($('#ingredients_slideshow--'+sectionId).hasClass('swiper-enabled')) {
//         var mySwiperGrid = document.querySelector('#ingredients-carousel-'+sectionId).swiper;
//         mySwiperGrid.slideToLoop(mySwiperIndex, 300);
//       }
//     };

  }
  return HomeIngredientsSlideshow;
})();
theme.HomeTestimonials  = (function() {
  function HomeTestimonials(container) {
    var $container = this.$container = $(container),
        sectionId = this.sectionId = $container.attr('data-section-id'),
        Carousel = this.Carousel = $container.find('.swiper-container'),
        next = '#swiper-button-next-'+sectionId,
        prev = '#swiper-button-prev-'+sectionId;
    var mySwiper = new Swiper (Carousel, {
      autoHeight: false,
...
$(function() {
  ...
  sections.register('ingredients_slideshow', theme.HomeIngredientsSlideshow);
  sections.register('homepage-testimonials', theme.HomeTestimonials);
...
//	Swiper 5.3.6 | http://www.idangero.us/swiper/ | Copyright 2014-2018 Vladimir Kharlampidi | Released under the MIT License
...
//	fancyBox v3.5.7 | http://fancyapps.com/fancybox/ | Copyright 2019 fancyApps
...
!function(a){"function"==typeof define&&define.amd?define(["jquery"],a):"object"==typeof module&&module.exports?module.exports=a(require("jquery")):a(jQuery)}(function(a){a.extend(a.fn,{validate:function(b){if(!this.length)return void(b&&b.debug&&window.console&&console.warn("Nothing selected, can't validate, returning nothing."));var c=a.data(this[0],"validator");return c?c:(this.attr("novalidate","novalidate"),c=new a.validator(b,this[0]),a.data(this[0],"validator",c),c.settings.onsubmit&&(this.on("click.validate",":submit",function(b){c.submitButton=b.currentTarget,a(this).hasClass("cancel")&&(c.cancelSubmit=!0),void 0!==a(this).attr("formnovalidate")&&(c.cancelSubmit=!0)}),this.on("submit.validate",function(b){function d(){var d,e;return c.submitButton&&(c.settings.submitHandler||c.formSubmitted)&&(d=a("<input type='hidden'/>").attr("name",c.submitButton.name).val(a(c.submitButton).val()).appendTo(c.currentForm)),!(c.settings.submitHandler&&!c.settings.debug)||(e=c.settings.submitHandler.call(c,c.currentForm,b),d&&d.remove(),void 0!==e&&e)}return c.settings.debug&&b.preventDefault(),c.cancelSubmit?(c.cancelSubmit=!1,d()):c.form()?c.pendingRequest?(c.formSubmitted=!0,!1):d():(c.focusInvalid(),!1)})),c)},valid:function(){var b,c,d;return a(this[0]).is("form")?b=this.validate().form():(d=[],b=!0,c=a(this[0].form).validate(),this.each(function(){b=c.element(this)&&b,b||(d=d.concat(c.errorList))}),c.errorList=d),b},rules:function(b,c){var d,e,f,g,h,i,j=this[0],k="undefined"!=typeof this.attr("contenteditable")&&"false"!==this.attr("contenteditable");if(null!=j&&(!j.form&&k&&(j.form=this.closest("form")[0],j.name=this.attr("name")),null!=j.form)){if(b)switch(d=a.data(j.form,"validator").settings,e=d.rules,f=a.validator.staticRules(j),b){case"add":a.extend(f,a.validator.normalizeRule(c)),delete f.messages,e[j.name]=f,c.messages&&(d.messages[j.name]=a.extend(d.messages[j.name],c.messages));break;case"remove":return c?(i={},a.each(c.split(/\s/),function(a,b){i[b]=f[b],delete f[b]}),i):(delete e[j.name],f)}return g=a.validator.normalizeRules(a.extend({},a.validator.classRules(j),a.validator.attributeRules(j),a.validator.dataRules(j),a.validator.staticRules(j)),j),g.required&&(h=g.required,delete g.required,g=a.extend({required:h},g)),g.remote&&(h=g.remote,delete g.remote,g=a.extend(g,{remote:h})),g}}}),a.extend(a.expr.pseudos||a.expr[":"],{blank:function(b){return!a.trim(""+a(b).val())},filled:function(b){var c=a(b).val();return null!==c&&!!a.trim(""+c)},unchecked:function(b){return!a(b).prop("checked")}}),a.validator=function(b,c){this.settings=a.extend(!0,{},a.validator.defaults,b),this.currentForm=c,this.init()},a.validator.format=function(b,c){return 1===arguments.length?function(){var c=a.makeArray(arguments);return c.unshift(b),a.validator.format.apply(this,c)}:void 0===c?b:(arguments.length>2&&c.constructor!==Array&&(c=a.makeArray(arguments).slice(1)),c.constructor!==Array&&(c=[c]),a.each(c,function(a,c){b=b.replace(new RegExp("\\{"+a+"\\}","g"),function(){return c})}),b)},a.extend(a.validator,{defaults:{messages:{},groups:{},rules:{},errorClass:"error",pendingClass:"pending",validClass:"valid",errorElement:"label",focusCleanup:!1,focusInvalid:!0,errorContainer:a([]),errorLabelContainer:a([]),onsubmit:!0,ignore:":hidden",ignoreTitle:!1,onfocusin:function(a){this.lastActive=a,this.settings.focusCleanup&&(this.settings.unhighlight&&this.settings.unhighlight.call(this,a,this.settings.errorClass,this.settings.validClass),this.hideThese(this.errorsFor(a)))},onfocusout:function(a){this.checkable(a)||!(a.name in this.submitted)&&this.optional(a)||this.element(a)},onkeyup:function(b,c){var d=[16,17,18,20,35,36,37,38,39,40,45,144,225];9===c.which&&""===this.elementValue(b)||a.inArray(c.keyCode,d)!==-1||(b.name in this.submitted||b.name in this.invalid)&&this.element(b)},onclick:function(a){a.name in this.submitted?this.element(a):a.parentNode.name in this.submitted&&this.element(a.parentNode)},highlight:function(b,c,d){"radio"===b.type?this.findByName(b.name).addClass(c).removeClass(d):a(b).addClass(c).removeClass(d)},unhighlight:function(b,c,d){"radio"===b.type?this.findByName(b.name).removeClass(c).addClass(d):a(b).removeClass(c).addClass(d)}},setDefaults:function(b){a.extend(a.validator.defaults,b)},messages:{required:theme.language.general_validation_required,remote:theme.language.general_validation_fix_field,email:theme.language.general_validation_valid_email,url:theme.language.general_validation_valid_url,date:theme.language.general_validation_valid_date,dateISO:theme.language.general_validation_valid_iso,number:theme.language.general_validation_valid_number,digits:theme.language.general_validation_only_digits,equalTo:theme.language.general_validation_same_value,maxlength:a.validator.format(theme.language. general_validation_characters_exceed + ' {0}'),minlength:a.validator.format(theme.language.general_validation_characters_min +' {0}'),rangelength:a.validator.format(theme.language.general_validation_characters_range + ' {0} - {1}'),range:a.validator.format(theme.language.general_validation_value_range + ' {0} - {1}'),max:a.validator.format(theme.language.general_validation_value_less + ' {0}'),min:a.validator.format(theme.language.general_validation_value_greater + ' {0}'),step:a.validator.format(theme.language.general_validation_multiple + ' {0}')},autoCreateRanges:!1,prototype:{init:function(){function b(b){var c="undefined"!=typeof a(this).attr("contenteditable")&&"false"!==a(this).attr("contenteditable");if(!this.form&&c&&(this.form=a(this).closest("form")[0],this.name=a(this).attr("name")),d===this.form){var e=a.data(this.form,"validator"),f="on"+b.type.replace(/^validate/,""),g=e.settings;g[f]&&!a(this).is(g.ignore)&&g[f].call(e,this,b)}}this.labelContainer=a(this.settings.errorLabelContainer),this.errorContext=this.labelContainer.length&&this.labelContainer||a(this.currentForm),this.containers=a(this.settings.errorContainer).add(this.settings.errorLabelContainer),this.submitted={},this.valueCache={},this.pendingRequest=0,this.pending={},this.invalid={},this.reset();var c,d=this.currentForm,e=this.groups={};a.each(this.settings.groups,function(b,c){"string"==typeof c&&(c=c.split(/\s/)),a.each(c,function(a,c){e[c]=b})}),c=this.settings.rules,a.each(c,function(b,d){c[b]=a.validator.normalizeRule(d)}),a(this.currentForm).on("focusin.validate focusout.validate keyup.validate",":text, [type='password'], [type='file'], select, textarea, [type='number'], [type='search'], [type='tel'], [type='url'], [type='email'], [type='datetime'], [type='date'], [type='month'], [type='week'], [type='time'], [type='datetime-local'], [type='range'], [type='color'], [type='radio'], [type='checkbox'], [contenteditable], [type='button']",b).on("click.validate","select, option, [type='radio'], [type='checkbox']",b),this.settings.invalidHandler&&a(this.currentForm).on("invalid-form.validate",this.settings.invalidHandler)},form:function(){return this.checkForm(),a.extend(this.submitted,this.errorMap),this.invalid=a.extend({},this.errorMap),this.valid()||a(this.currentForm).triggerHandler("invalid-form",[this]),this.showErrors(),this.valid()},checkForm:function(){this.prepareForm();for(var a=0,b=this.currentElements=this.elements();b[a];a++)this.check(b[a]);return this.valid()},element:function(b){var c,d,e=this.clean(b),f=this.validationTargetFor(e),g=this,h=!0;return void 0===f?delete this.invalid[e.name]:(this.prepareElement(f),this.currentElements=a(f),d=this.groups[f.name],d&&a.each(this.groups,function(a,b){b===d&&a!==f.name&&(e=g.validationTargetFor(g.clean(g.findByName(a))),e&&e.name in g.invalid&&(g.currentElements.push(e),h=g.check(e)&&h))}),c=this.check(f)!==!1,h=h&&c,c?this.invalid[f.name]=!1:this.invalid[f.name]=!0,this.numberOfInvalids()||(this.toHide=this.toHide.add(this.containers)),this.showErrors(),a(b).attr("aria-invalid",!c)),h},showErrors:function(b){if(b){var c=this;a.extend(this.errorMap,b),this.errorList=a.map(this.errorMap,function(a,b){return{message:a,element:c.findByName(b)[0]}}),this.successList=a.grep(this.successList,function(a){return!(a.name in b)})}this.settings.showErrors?this.settings.showErrors.call(this,this.errorMap,this.errorList):this.defaultShowErrors()},resetForm:function(){a.fn.resetForm&&a(this.currentForm).resetForm(),this.invalid={},this.submitted={},this.prepareForm(),this.hideErrors();var b=this.elements().removeData("previousValue").removeAttr("aria-invalid");this.resetElements(b)},resetElements:function(a){var b;if(this.settings.unhighlight)for(b=0;a[b];b++)this.settings.unhighlight.call(this,a[b],this.settings.errorClass,""),this.findByName(a[b].name).removeClass(this.settings.validClass);else a.removeClass(this.settings.errorClass).removeClass(this.settings.validClass)},numberOfInvalids:function(){return this.objectLength(this.invalid)},objectLength:function(a){var b,c=0;for(b in a)void 0!==a[b]&&null!==a[b]&&a[b]!==!1&&c++;return c},hideErrors:function(){this.hideThese(this.toHide)},hideThese:function(a){a.not(this.containers).text(""),this.addWrapper(a).hide()},valid:function(){return 0===this.size()},size:function(){return this.errorList.length},focusInvalid:function(){if(this.settings.focusInvalid)try{a(this.findLastActive()||this.errorList.length&&this.errorList[0].element||[]).filter(":visible").trigger("focus").trigger("focusin")}catch(b){}},findLastActive:function(){var b=this.lastActive;return b&&1===a.grep(this.errorList,function(a){return a.element.name===b.name}).length&&b},elements:function(){var b=this,c={};return a(this.currentForm).find("input, select, textarea, [contenteditable]").not(":submit, :reset, :image, :disabled").not(this.settings.ignore).filter(function(){var d=this.name||a(this).attr("name"),e="undefined"!=typeof a(this).attr("contenteditable")&&"false"!==a(this).attr("contenteditable");return!d&&b.settings.debug&&window.console&&console.error("%o has no name assigned",this),e&&(this.form=a(this).closest("form")[0],this.name=d),this.form===b.currentForm&&(!(d in c||!b.objectLength(a(this).rules()))&&(c[d]=!0,!0))})},clean:function(b){return a(b)[0]},errors:function(){var b=this.settings.errorClass.split(" ").join(".");return a(this.settings.errorElement+"."+b,this.errorContext)},resetInternals:function(){this.successList=[],this.errorList=[],this.errorMap={},this.toShow=a([]),this.toHide=a([])},reset:function(){this.resetInternals(),this.currentElements=a([])},prepareForm:function(){this.reset(),this.toHide=this.errors().add(this.containers)},prepareElement:function(a){this.reset(),this.toHide=this.errorsFor(a)},elementValue:function(b){var c,d,e=a(b),f=b.type,g="undefined"!=typeof e.attr("contenteditable")&&"false"!==e.attr("contenteditable");return"radio"===f||"checkbox"===f?this.findByName(b.name).filter(":checked").val():"number"===f&&"undefined"!=typeof b.validity?b.validity.badInput?"NaN":e.val():(c=g?e.text():e.val(),"file"===f?"C:\\fakepath\\"===c.substr(0,12)?c.substr(12):(d=c.lastIndexOf("/"),d>=0?c.substr(d+1):(d=c.lastIndexOf("\\"),d>=0?c.substr(d+1):c)):"string"==typeof c?c.replace(/\r/g,""):c)},check:function(b){b=this.validationTargetFor(this.clean(b));var c,d,e,f,g=a(b).rules(),h=a.map(g,function(a,b){return b}).length,i=!1,j=this.elementValue(b);"function"==typeof g.normalizer?f=g.normalizer:"function"==typeof this.settings.normalizer&&(f=this.settings.normalizer),f&&(j=f.call(b,j),delete g.normalizer);for(d in g){e={method:d,parameters:g[d]};try{if(c=a.validator.methods[d].call(this,j,b,e.parameters),"dependency-mismatch"===c&&1===h){i=!0;continue}if(i=!1,"pending"===c)return void(this.toHide=this.toHide.not(this.errorsFor(b)));if(!c)return this.formatAndAdd(b,e),!1}catch(k){throw this.settings.debug&&window.console&&console.log("Exception occurred when checking element "+b.id+", check the '"+e.method+"' method.",k),k instanceof TypeError&&(k.message+=".  Exception occurred when checking element "+b.id+", check the '"+e.method+"' method."),k}}if(!i)return this.objectLength(g)&&this.successList.push(b),!0},customDataMessage:function(b,c){return a(b).data("msg"+c.charAt(0).toUpperCase()+c.substring(1).toLowerCase())||a(b).data("msg")},customMessage:function(a,b){var c=this.settings.messages[a];return c&&(c.constructor===String?c:c[b])},findDefined:function(){for(var a=0;a<arguments.length;a++)if(void 0!==arguments[a])return arguments[a]},defaultMessage:function(b,c){"string"==typeof c&&(c={method:c});var d=this.findDefined(this.customMessage(b.name,c.method),this.customDataMessage(b,c.method),!this.settings.ignoreTitle&&b.title||void 0,a.validator.messages[c.method],"<strong>Warning: No message defined for "+b.name+"</strong>"),e=/\$?\{(\d+)\}/g;return"function"==typeof d?d=d.call(this,c.parameters,b):e.test(d)&&(d=a.validator.format(d.replace(e,"{$1}"),c.parameters)),d},formatAndAdd:function(a,b){var c=this.defaultMessage(a,b);this.errorList.push({message:c,element:a,method:b.method}),this.errorMap[a.name]=c,this.submitted[a.name]=c},addWrapper:function(a){return this.settings.wrapper&&(a=a.add(a.parent(this.settings.wrapper))),a},defaultShowErrors:function(){var a,b,c;for(a=0;this.errorList[a];a++)c=this.errorList[a],this.settings.highlight&&this.settings.highlight.call(this,c.element,this.settings.errorClass,this.settings.validClass),this.showLabel(c.element,c.message);if(this.errorList.length&&(this.toShow=this.toShow.add(this.containers)),this.settings.success)for(a=0;this.successList[a];a++)this.showLabel(this.successList[a]);if(this.settings.unhighlight)for(a=0,b=this.validElements();b[a];a++)this.settings.unhighlight.call(this,b[a],this.settings.errorClass,this.settings.validClass);this.toHide=this.toHide.not(this.toShow),this.hideErrors(),this.addWrapper(this.toShow).show()},validElements:function(){return this.currentElements.not(this.invalidElements())},invalidElements:function(){return a(this.errorList).map(function(){return this.element})},showLabel:function(b,c){var d,e,f,g,h=this.errorsFor(b),i=this.idOrName(b),j=a(b).attr("aria-describedby");h.length?(h.removeClass(this.settings.validClass).addClass(this.settings.errorClass),h.html(c)):(h=a("<"+this.settings.errorElement+">").attr("id",i+"-error").addClass(this.settings.errorClass).html(c||""),d=h,this.settings.wrapper&&(d=h.hide().show().wrap("<"+this.settings.wrapper+"/>").parent()),this.labelContainer.length?this.labelContainer.append(d):this.settings.errorPlacement?this.settings.errorPlacement.call(this,d,a(b)):d.insertAfter(b),h.is("label")?h.attr("for",i):0===h.parents("label[for='"+this.escapeCssMeta(i)+"']").length&&(f=h.attr("id"),j?j.match(new RegExp("\\b"+this.escapeCssMeta(f)+"\\b"))||(j+=" "+f):j=f,a(b).attr("aria-describedby",j),e=this.groups[b.name],e&&(g=this,a.each(g.groups,function(b,c){c===e&&a("[name='"+g.escapeCssMeta(b)+"']",g.currentForm).attr("aria-describedby",h.attr("id"))})))),!c&&this.settings.success&&(h.text(""),"string"==typeof this.settings.success?h.addClass(this.settings.success):this.settings.success(h,b)),this.toShow=this.toShow.add(h)},errorsFor:function(b){var c=this.escapeCssMeta(this.idOrName(b)),d=a(b).attr("aria-describedby"),e="label[for='"+c+"'], label[for='"+c+"'] *";return d&&(e=e+", #"+this.escapeCssMeta(d).replace(/\s+/g,", #")),this.errors().filter(e)},escapeCssMeta:function(a){return a.replace(/([\\!"#$%&'()*+,.\/:;<=>?@\[\]^`{|}~])/g,"\\$1")},idOrName:function(a){return this.groups[a.name]||(this.checkable(a)?a.name:a.id||a.name)},validationTargetFor:function(b){return this.checkable(b)&&(b=this.findByName(b.name)),a(b).not(this.settings.ignore)[0]},checkable:function(a){return/radio|checkbox/i.test(a.type)},findByName:function(b){return a(this.currentForm).find("[name='"+this.escapeCssMeta(b)+"']")},getLength:function(b,c){switch(c.nodeName.toLowerCase()){case"select":return a("option:selected",c).length;case"input":if(this.checkable(c))return this.findByName(c.name).filter(":checked").length}return b.length},depend:function(a,b){return!this.dependTypes[typeof a]||this.dependTypes[typeof a](a,b)},dependTypes:{"boolean":function(a){return a},string:function(b,c){return!!a(b,c.form).length},"function":function(a,b){return a(b)}},optional:function(b){var c=this.elementValue(b);return!a.validator.methods.required.call(this,c,b)&&"dependency-mismatch"},startRequest:function(b){this.pending[b.name]||(this.pendingRequest++,a(b).addClass(this.settings.pendingClass),this.pending[b.name]=!0)},stopRequest:function(b,c){this.pendingRequest--,this.pendingRequest<0&&(this.pendingRequest=0),delete this.pending[b.name],a(b).removeClass(this.settings.pendingClass),c&&0===this.pendingRequest&&this.formSubmitted&&this.form()?(a(this.currentForm).submit(),this.submitButton&&a("input:hidden[name='"+this.submitButton.name+"']",this.currentForm).remove(),this.formSubmitted=!1):!c&&0===this.pendingRequest&&this.formSubmitted&&(a(this.currentForm).triggerHandler("invalid-form",[this]),this.formSubmitted=!1)},previousValue:function(b,c){return c="string"==typeof c&&c||"remote",a.data(b,"previousValue")||a.data(b,"previousValue",{old:null,valid:!0,message:this.defaultMessage(b,{method:c})})},destroy:function(){this.resetForm(),a(this.currentForm).off(".validate").removeData("validator").find(".validate-equalTo-blur").off(".validate-equalTo").removeClass("validate-equalTo-blur").find(".validate-lessThan-blur").off(".validate-lessThan").removeClass("validate-lessThan-blur").find(".validate-lessThanEqual-blur").off(".validate-lessThanEqual").removeClass("validate-lessThanEqual-blur").find(".validate-greaterThanEqual-blur").off(".validate-greaterThanEqual").removeClass("validate-greaterThanEqual-blur").find(".validate-greaterThan-blur").off(".validate-greaterThan").removeClass("validate-greaterThan-blur")}},classRuleSettings:{required:{required:!0},email:{email:!0},url:{url:!0},date:{date:!0},dateISO:{dateISO:!0},number:{number:!0},digits:{digits:!0},creditcard:{creditcard:!0}},addClassRules:function(b,c){b.constructor===String?this.classRuleSettings[b]=c:a.extend(this.classRuleSettings,b)},classRules:function(b){var c={},d=a(b).attr("class");return d&&a.each(d.split(" "),function(){this in a.validator.classRuleSettings&&a.extend(c,a.validator.classRuleSettings[this])}),c},normalizeAttributeRule:function(a,b,c,d){/min|max|step/.test(c)&&(null===b||/number|range|text/.test(b))&&(d=Number(d),isNaN(d)&&(d=void 0)),d||0===d?a[c]=d:b===c&&"range"!==b&&(a[c]=!0)},attributeRules:function(b){var c,d,e={},f=a(b),g=b.getAttribute("type");for(c in a.validator.methods)"required"===c?(d=b.getAttribute(c),""===d&&(d=!0),d=!!d):d=f.attr(c),this.normalizeAttributeRule(e,g,c,d);return e.maxlength&&/-1|2147483647|524288/.test(e.maxlength)&&delete e.maxlength,e},dataRules:function(b){var c,d,e={},f=a(b),g=b.getAttribute("type");for(c in a.validator.methods)d=f.data("rule"+c.charAt(0).toUpperCase()+c.substring(1).toLowerCase()),""===d&&(d=!0),this.normalizeAttributeRule(e,g,c,d);return e},staticRules:function(b){var c={},d=a.data(b.form,"validator");return d.settings.rules&&(c=a.validator.normalizeRule(d.settings.rules[b.name])||{}),c},normalizeRules:function(b,c){return a.each(b,function(d,e){if(e===!1)return void delete b[d];if(e.param||e.depends){var f=!0;switch(typeof e.depends){case"string":f=!!a(e.depends,c.form).length;break;case"function":f=e.depends.call(c,c)}f?b[d]=void 0===e.param||e.param:(a.data(c.form,"validator").resetElements(a(c)),delete b[d])}}),a.each(b,function(d,e){b[d]=a.isFunction(e)&&"normalizer"!==d?e(c):e}),a.each(["minlength","maxlength"],function(){b[this]&&(b[this]=Number(b[this]))}),a.each(["rangelength","range"],function(){var c;b[this]&&(a.isArray(b[this])?b[this]=[Number(b[this][0]),Number(b[this][1])]:"string"==typeof b[this]&&(c=b[this].replace(/[\[\]]/g,"").split(/[\s,]+/),b[this]=[Number(c[0]),Number(c[1])]))}),a.validator.autoCreateRanges&&(null!=b.min&&null!=b.max&&(b.range=[b.min,b.max],delete b.min,delete b.max),null!=b.minlength&&null!=b.maxlength&&(b.rangelength=[b.minlength,b.maxlength],delete b.minlength,delete b.maxlength)),b},normalizeRule:function(b){if("string"==typeof b){var c={};a.each(b.split(/\s/),function(){c[this]=!0}),b=c}return b},addMethod:function(b,c,d){a.validator.methods[b]=c,a.validator.messages[b]=void 0!==d?d:a.validator.messages[b],c.length<3&&a.validator.addClassRules(b,a.validator.normalizeRule(b))},methods:{required:function(b,c,d){if(!this.depend(d,c))return"dependency-mismatch";if("select"===c.nodeName.toLowerCase()){var e=a(c).val();return e&&e.length>0}return this.checkable(c)?this.getLength(b,c)>0:void 0!==b&&null!==b&&b.length>0},email:function(a,b){return this.optional(b)||/^[a-zA-Z0-9.!#$%&'*+\/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/.test(a)},url:function(a,b){return this.optional(b)||/^(?:(?:(?:https?|ftp):)?\/\/)(?:\S+(?::\S*)?@)?(?:(?!(?:10|127)(?:\.\d{1,3}){3})(?!(?:169\.254|192\.168)(?:\.\d{1,3}){2})(?!172\.(?:1[6-9]|2\d|3[0-1])(?:\.\d{1,3}){2})(?:[1-9]\d?|1\d\d|2[01]\d|22[0-3])(?:\.(?:1?\d{1,2}|2[0-4]\d|25[0-5])){2}(?:\.(?:[1-9]\d?|1\d\d|2[0-4]\d|25[0-4]))|(?:(?:[a-z\u00a1-\uffff0-9]-*)*[a-z\u00a1-\uffff0-9]+)(?:\.(?:[a-z\u00a1-\uffff0-9]-*)*[a-z\u00a1-\uffff0-9]+)*(?:\.(?:[a-z\u00a1-\uffff]{2,})).?)(?::\d{2,5})?(?:[\/?#]\S*)?$/i.test(a)},date:function(){var a=!1;return function(b,c){return a||(a=!0,this.settings.debug&&window.console&&console.warn("The `date` method is deprecated and will be removed in version '2.0.0'.\nPlease don't use it, since it relies on the Date constructor, which\nbehaves very differently across browsers and locales. Use `dateISO`\ninstead or one of the locale specific methods in `localizations/`\nand `additional-methods.js`.")),this.optional(c)||!/Invalid|NaN/.test(new Date(b).toString())}}(),dateISO:function(a,b){return this.optional(b)||/^\d{4}[\/\-](0?[1-9]|1[012])[\/\-](0?[1-9]|[12][0-9]|3[01])$/.test(a)},number:function(a,b){return this.optional(b)||/^(?:-?\d+|-?\d{1,3}(?:,\d{3})+)?(?:\.\d+)?$/.test(a)},digits:function(a,b){return this.optional(b)||/^\d+$/.test(a)},minlength:function(b,c,d){var e=a.isArray(b)?b.length:this.getLength(b,c);return this.optional(c)||e>=d},maxlength:function(b,c,d){var e=a.isArray(b)?b.length:this.getLength(b,c);return this.optional(c)||e<=d},rangelength:function(b,c,d){var e=a.isArray(b)?b.length:this.getLength(b,c);return this.optional(c)||e>=d[0]&&e<=d[1]},min:function(a,b,c){return this.optional(b)||a>=c},max:function(a,b,c){return this.optional(b)||a<=c},range:function(a,b,c){return this.optional(b)||a>=c[0]&&a<=c[1]},step:function(b,c,d){var e,f=a(c).attr("type"),g="Step attribute on input type "+f+" is not supported.",h=["text","number","range"],i=new RegExp("\\b"+f+"\\b"),j=f&&!i.test(h.join()),k=function(a){var b=(""+a).match(/(?:\.(\d+))?$/);return b&&b[1]?b[1].length:0},l=function(a){return Math.round(a*Math.pow(10,e))},m=!0;if(j)throw new Error(g);return e=k(d),(k(b)>e||l(b)%l(d)!==0)&&(m=!1),this.optional(c)||m},equalTo:function(b,c,d){var e=a(d);return this.settings.onfocusout&&e.not(".validate-equalTo-blur").length&&e.addClass("validate-equalTo-blur").on("blur.validate-equalTo",function(){a(c).valid()}),b===e.val()},remote:function(b,c,d,e){if(this.optional(c))return"dependency-mismatch";e="string"==typeof e&&e||"remote";var f,g,h,i=this.previousValue(c,e);return this.settings.messages[c.name]||(this.settings.messages[c.name]={}),i.originalMessage=i.originalMessage||this.settings.messages[c.name][e],this.settings.messages[c.name][e]=i.message,d="string"==typeof d&&{url:d}||d,h=a.param(a.extend({data:b},d.data)),i.old===h?i.valid:(i.old=h,f=this,this.startRequest(c),g={},g[c.name]=b,a.ajax(a.extend(!0,{mode:"abort",port:"validate"+c.name,dataType:"json",data:g,context:f.currentForm,success:function(a){var d,g,h,j=a===!0||"true"===a;f.settings.messages[c.name][e]=i.originalMessage,j?(h=f.formSubmitted,f.resetInternals(),f.toHide=f.errorsFor(c),f.formSubmitted=h,f.successList.push(c),f.invalid[c.name]=!1,f.showErrors()):(d={},g=a||f.defaultMessage(c,{method:e,parameters:b}),d[c.name]=i.message=g,f.invalid[c.name]=!0,f.showErrors(d)),i.valid=j,f.stopRequest(c,j)}},d)),"pending")}}});var b,c={};return a.ajaxPrefilter?a.ajaxPrefilter(function(a,b,d){var e=a.port;"abort"===a.mode&&(c[e]&&c[e].abort(),c[e]=d)}):(b=a.ajax,a.ajax=function(d){var e=("mode"in d?d:a.ajaxSettings).mode,f=("port"in d?d:a.ajaxSettings).port;return"abort"===e?(c[f]&&c[f].abort(),c[f]=b.apply(this,arguments),c[f]):b.apply(this,arguments)}),a});
  ...
  $('body').on('click', '.additional_details_anchor', function(e) {
    ...
    var tabs = $('.custom_tabs');
    if(tabs.length) {

      var header_height = $('header').innerHeight();

      var top = tabs.offset().top - header_height - 20;
  ...
  $('body').on('click', '.custom_tabs  .tabs_menu a', function(e) {
    ...
    var parent = $(this).closest('.custom_tabs');
    var index = $(this).attr('data-index');
    var href = $(this).attr('data-href');
    var tabs_menu = parent.find('.tabs_menu');
    var tabs_content = parent.find('.tabs_content');

    tabs_menu.find('a.active').removeClass('active');
    tabs_content.find('.custom-tab-body-content.active').removeClass('active');
  ...
  $('body').on('click', '.mobile_accordion_title', function(e) {
    e.preventDefault();
    var parent = $(this).closest('.custom_tabs .mobile_tabs');
    var index = $(this).attr('data-index');
    var href = $(this).attr('data-href');
    var tabs_content = parent.find('.tabs_content');
    ...
    if($(this).hasClass('active')) {
      parent.find(href).slideUp();
      $(this).removeClass('active');
    } else {
      parent.find('.custom-tab-body-content.active_tab').slideUp().removeClass('active_tab');
      tabs_content.find('.mobile_accordion_title.active').removeClass('active');

      parent.find(href).addClass('active_tab').slideDown();
      $(this).addClass('active');
    }

  });
  $('body').on('click', '.recent-blog  .right-category .blog-subTitle', function(e) {
    e.preventDefault();
  ...
  if($('.recent-blog  .right-category .blog-subTitle').length) {
    $(window).resize(function () {
      if($(window).width() >= 992) {
        $('.recent-blog  .right-category .blog-subTitle').removeClass('open');
        $('#category-list').removeAttr('style');
      }
    });
  }
  var page_iframe_video = $('.custom_page_template .main-video #video');
  if(page_iframe_video.length) {
    $('body').on('click', '.play-btn', function(e) {
      e.preventDefault();

      var videoIframe = $(this).closest('.main-video').find('iframe');
      var videoIframeSrc = videoIframe.attr('src');
      videoIframeSrc = videoIframeSrc + "&autoPlay=true";
      videoIframe.addClass('openVideo');
      videoIframe.attr('src',videoIframeSrc );
      $(this).hide();

    });
  }
  var faq_tabs = $('.custom_page_template .faq-info .nav-tabs');
  if(faq_tabs.length) {
    $('body').on('click', '.custom_page_template .faq-info .nav-tabs .nav-link', function(e) {
      $(this).off();
      e.preventDefault();

      var parent = $(this).closest('.faq-area');
      var href = $(this).attr('href');
      var tabs_menu = parent.find('.nav-tabs');
      var tabs_content = parent.find('.tab-content');

      tabs_menu.find('a.active').removeClass('active');
      tabs_content.find('.card.tab-pane.active.show').removeClass('active').removeClass('show');
      parent.find(href).addClass('active').addClass('show');
      $(this).addClass('active');

      return false;

    });
    $('body').on('click', '.custom_page_template .tab-pane .card-header', function(e) {
      e.preventDefault();

      var card_area = $(this).closest('.card-area');
      var card_body = $(this).closest('.card-body');
      var collapse  = $(this).next('.collapse');

      if(card_area.hasClass('active')) {
        collapse.slideUp(400, function() {
          $(this).removeClass('show');
        });
        card_area.removeClass('active');
      } else {
        card_body.find('.card-area.active').find('.collapse').slideUp().removeClass('show');
        card_body.find('.card-area.active').removeClass('active');
        collapse.slideDown(400, function() {
          $(this).addClass('show');
          card_area.addClass('active');
          var sticky_header = $('#shopify-section-header header');
          var sticky_header_height = sticky_header.innerHeight();
          var offsetTop = card_area.offset().top - sticky_header_height - 15;
    ...
    $('body').on('click', '.tab-content .hidetab', function(e) {
      ...
      var href = $(this).attr('href');
      var tabs_content = parent.closest('.tab-content');
      var collapse_block = parent.find('[role="tabpanel"]');


      if($(this).hasClass('collapsed')) {
        parent.find(href).slideUp(400, function() {
          $(this).removeClass('show');
        });
        $(this).removeClass('collapsed');
      } else {
        tabs_content.find('.collapse.show').slideUp(400, function() {
          $(this).removeClass('show');
        });
        tabs_content.find('.hidetab.collapsed').removeClass('collapsed');

        parent.find(href).slideDown(400, function() {
          $(this).addClass('show');
        });
  ...
  var list_anchor = $('.redbarn-team .list-anchor');
  if(list_anchor.length) {
    ...
    $('body').on('click','.custom_page_template .close-modal', function(e) {
      e.preventDefault();
    ...
    $('body').on('click','.modal.team', function(e) {
      var el = $(e.target);
      if(el.hasClass('modal-dialog') || el.hasClass('modal')) {
  ...
  var cookie_notice = localStorage.getItem('cookie_notice');
  ...
  var video_with_text_section_button = $('.custom_page_template .video_with_text .video_block .play_btn');
  if(video_with_text_section_button.length) {
    video_with_text_section_button.on('click', function(e) {
      e.preventDefault();

      var video_block  = $(this).closest('.video_block');
      var video_iframe = video_block.find('iframe');
      var videoIframeSrc = video_iframe.attr('src');

      videoIframeSrc = videoIframeSrc + "&autoplay=1";
      video_iframe.attr('src',videoIframeSrc );
  ...
  $(".redbarn-ingredient .input-formArea form").on("submit", function(e){
   ...
   var value = $(this).find(".form-control").val();
    ...
    var video_btn_icon  = $('.video_control_btn .video_btn_icon');
  if(video_btn_icon.length) {
    video_btn_icon.on('click', function(e) {
      e.preventDefault();
      var video_control_btn = $(this).closest('.video_control_btn');
      var video = $(this).closest('.video_wrapper').find('video');
      video[0].play();
      video_control_btn.hide();
    });

    var videoTag = $('.video_wrapper video');
    videoTag.on('pause', function(e) {
  ...
  $('body').on('click', '.store_locator_link', function(e) {
    e.preventDefault();
    var search_params = $(this).attr("data-search-params");
    if(search_params != ""){
      window.location.href = 'https://redbarn-shop.myshopify.com/pages/store-locator?stockist-tags='+search_params;
    }else{
      window.location.href = 'https://redbarn-shop.myshopify.com/pages/store-locator';
    }
  ...
  if ($('body').hasClass('template-article')) {
    var loadProductRecommendationsIntoSection = function() {
      ...
      var recommendedType = productRecommendationsSection.dataset.type;
      if (recommendedType == 'collection_handle') {
        ...
        request.onload = function() {
          if (request.status >= 200 && request.status < 300) {
            ...
            container.innerHTML = request.response;
            productRecommendationsSection.innerHTML = container.querySelector('.product-recommendations').innerHTML;
            if (container.querySelector('.product-loop-product-template') != null) {
            	$('.product-loop-product-template').addClass('product-loop-article-template');
            }
            if (productRecommendationsSection.children.length < 2) {
              $('#recommended').addClass('hidden')
            }
  ...
  $('body').on('click', '#cart-notification .modal__close-button', function(e) {
  	e.preventDefault();
    $(this).closest('#cart-notification').removeClass('active');
  });
  $('#cart-count-desktop').on('mouseover', function(e) {
  	var cart_notificationEl = $('#cart-notification');
    if(cart_notificationEl.length) {
      if(cart_notificationEl.hasClass('active')) {
      	cart_notificationEl.removeClass('active');
      }
    }
  });
});
